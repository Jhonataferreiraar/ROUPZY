...
export default async function AdminLoginPage({ searchParams }) {
  ...
  return <main className="admin-login-page">
    <section className="admin-login-visual" aria-label="Visão geral da operação do Roupzy">
      ...
      <div className="admin-login-visual-top"><BrandMark className="admin-login-mark" size={31} /><span>ROUPZY / OPERAÇÃO</span><span>ACESSO RESTRITO</span></div>
    ...
    <section className="admin-login-panel">
      <div className="admin-login-panel-top"><Link className="admin-login-brand" href="/admin/login" aria-label="Roupzy, acesso administrativo"><BrandMark className="admin-login-brand-mark" size={34} /><strong>ROUPZY</strong></Link><Link className="admin-login-back" href="/">Voltar ao site <span className="ri-arrow" aria-hidden="true" /></Link></div>
      <div className="admin-login-content"><span className="admin-login-kicker">PAINEL DE ADMINISTRAÇÃO</span><h2>Bem-vindo<br />{' '}<em>de volta.</em></h2><p>Entre com uma conta autorizada para acessar o controle geral do Roupzy.</p>{denied ? <p className="admin-login-denied" role="alert">Essa conta não possui permissão administrativa.</p> : null}<div className="admin-login-status"><i aria-hidden="true" /> Sessão protegida e monitorada.</div><AuthForm endpoint="/api/auth/admin-sign-in" redirectTo={redirectTo} submitLabel="Entrar no painel" allowSocial={false} /><Link className="admin-login-forgot" href="/esqueci-minha-senha">Esqueci minha senha <span className="ri-arrow" aria-hidden="true" /></Link></div>
      <div className="admin-login-footer"><span>ACESSO DO CLIENTE</span><Link href="/login">Entrar no meu espaço <span className="ri-arrow" aria-hidden="true" /></Link></div>
    </section>
  </main>
}
