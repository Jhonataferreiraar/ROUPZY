...
export default async function CapsulesPage() {
  ...
  return <div className="app-page shell rv-module-page">
    ...
    <div className="rv-module-split">
      <section className="rv-module-panel"><div className="rv-module-panel-head"><div><span>SELEÇÃO ATUAL</span><h2>Cápsula-base</h2></div><strong>{capsule.length}/12 peças</strong></div>{capsule.length ? <div className="rv-capsule-list">{capsule.map((piece, index) => <Link href={`/app/closet/${piece.id}`} key={piece.id}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{piece.name}</strong><small>{clothingCategoryLabel(piece.category)} · {piece.colors?.[0]?.name || 'cor a confirmar'}</small></div><b>↗</b></Link>)}</div> : <div className="app-empty"><BrandMark className="app-empty-mark" size={44} /><strong>Sua cápsula começa no closet.</strong><p>Cadastre algumas peças para o Roupzy montar uma seleção real.</p><Link className="app-primary-button" href="/app/closet/adicionar">Adicionar primeira peça <span>↗</span></Link></div>}</section>
