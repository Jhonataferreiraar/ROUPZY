    ...
    <div className={'app-navigation' + (mobileOnly ? ' app-navigation-mobile' : '')} key={pathname}>
      <nav className="app-nav" aria-label="Navegação do espaço pessoal">
        {groups.map((group) => <div className="app-nav-group" key={group.label}>
          ...
          {group.links.map(([href, label, exact], index) => <Link className={isActive(pathname, href, exact) ? 'app-nav-link is-active' : 'app-nav-link'} href={href} aria-current={isActive(pathname, href, exact) ? 'page' : undefined} key={href}><span className="app-nav-index">{String(index + 1).padStart(2, '0')}</span><span>{label}</span><i className="ri-arrow" aria-hidden="true" /></Link>)}
        </div>)}
      </nav>
      ...
      {open ? <nav className="app-mobile-menu" id="app-mobile-menu" aria-label="Navegação móvel do espaço pessoal">
        ...
        {links.map(([href, label, exact], index) => <Link className={isActive(pathname, href, exact) ? 'is-active' : ''} href={href} aria-current={isActive(pathname, href, exact) ? 'page' : undefined} onClick={closeMenu} key={href}><span className="app-mobile-menu-index">{String(index + 1).padStart(2, '0')}</span>{label}<i className="ri-arrow" aria-hidden="true" /></Link>)}
      </nav> : null}
    </div>
  )
}
